...
{	
	...
	static std::unique_ptr<RegressionTree1D::Node> tree_regression_1d_without_pruning(const Eigen::Ref<Eigen::MatrixXd> X, const Eigen::Ref<Eigen::VectorXd> y, const unsigned int allowed_split_levels, const unsigned int min_sample_size)
	{
		const double mean = y.mean();
		const auto sse = (y.array() - mean).square().sum();
		const auto sample_size = y.size();
		if (!allowed_split_levels || sample_size < min_sample_size) {						
			...
			if (split.second == -std::numeric_limits<double>::infinity()) {
				...
				std::unique_ptr<RegressionTree1D::SplitNode> split_node(new RegressionTree1D::SplitNode(sse, split.second, split.first));				
				std::vector<std::pair<Eigen::Index, double>> features(sample_size);
