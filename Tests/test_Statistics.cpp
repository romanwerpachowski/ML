...
TEST(StatisticsTest, gini_index_constant_class)
{
	...
	ASSERT_EQ(0, gini_index(data.begin(), data.end()));
...
TEST(StatisticsTest, gini_index_two_equal_classes)
{
	...
	ASSERT_NEAR(0.5, gini_index(data.begin(), data.end()), 1e-15);
...
TEST(StatisticsTest, gini_index_two_unequal_classes)
{
	...
	ASSERT_NEAR(3./8, gini_index(data.begin(), data.end()), 1e-15);
...
TEST(StatisticsTest, gini_index_three_equal_classes)
{
	...
	ASSERT_NEAR(2./3, gini_index(data.begin(), data.end()), 1e-15);
...
TEST(StatisticsTest, gini_index_and_mode_constant_class)
{
	...
	auto actual = gini_index_and_mode(data.begin(), data.end());
	ASSERT_EQ(0, actual.first);
	ASSERT_EQ(1, actual.second);
	actual = gini_index_and_mode(data.begin(), data.end(), 2);
	ASSERT_EQ(0, actual.first);
	ASSERT_EQ(1, actual.second);
}
...
TEST(StatisticsTest, gini_index_and_mode_two_equal_classes)
{
	...
	actual = gini_index_and_mode(data.begin(), data.end(), 3);
	ASSERT_NEAR(0.5, actual.first, 1e-15);
	ASSERT_EQ(0, actual.second);
}
