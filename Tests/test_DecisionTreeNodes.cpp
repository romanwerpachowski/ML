...
TEST(DecisionTreeNodesTest, nodes)
{
	...
	ASSERT_EQ(0u, root->feature_index);
	...
	ASSERT_EQ(2u, next_split->count_lower_nodes());
	root->higher = std::move(next_split);
	ASSERT_EQ(4u, root->count_lower_nodes());
	ASSERT_EQ(3u, root->count_leaf_nodes());
	...
	ASSERT_EQ(1u, lowest_split_nodes.size());
	ASSERT_EQ(1u, lowest_split_nodes.count(next_split_ptr));
